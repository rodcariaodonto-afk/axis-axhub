export default function DashboardMockup() {
  const metrics = [
    ...
    { label: "Contas a receber", value: "R$ 96k", color: "text-white" },
    { label: "Pedidos em aberto", value: "42", color: "text-[#F97316]" },
    { label: "Leads ativos", value: "1.248", color: "text-[#25D366]" },
  ];
  ...
  return (
    <div className="relative">
        ...
        <div className="p-5 space-y-4">
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {metrics.map((m) => (
              <div key={m.label} className="rounded-xl bg-white/5 ring-1 ring-white/10 p-3">
                ...
                <p className={`mt-1 text-lg font-bold ${m.color}`} style={{ fontFamily: "Plus Jakarta Sans" }}>
                  {m.value}
                </p>
              </div>
            ))}
          ...
          <div className="rounded-xl bg-white/5 ring-1 ring-white/10 p-4">
            <div className="flex items-center justify-between mb-3">
              <p className="text-xs font-semibold text-slate-300">Pipeline e fluxo financeiro</p>
          ...
          <div className="flex flex-wrap gap-2">
            {["CRM ativo", "ERP ativo", "WhatsApp ativo", "Governança ativa"].map((s) => (
              <span
                key={s}
      ...
      {/* Floating IA card */}
      <div className="hidden sm:block absolute -bottom-6 -left-6 rounded-xl bg-white shadow-xl ring-1 ring-slate-200 p-4 max-w-[260px]">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-[11px] text-[#4B5563]">3 insights operacionais gerados</p>
          </div>
        </div>
      </div>
    </div>
